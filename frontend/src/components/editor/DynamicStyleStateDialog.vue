...
watch(
  () => [props.modelValue, props.state] as const,
  ([open, state]) => {
    if (!open || !state) return
    draft.name = state.name
    draft.scenarios = state.scenarios.map((scene) => ({
      ...scene,
      conditions: scene.conditions.map((cond) => ({ ...cond })),
    }))
    draft.styles = { ...state.styles }
...
function handleSave() {
  ...
  const name = draft.name.trim() || props.state.name
  emit('save', {
    id: props.state.id,
    name,
  ...
  >
    <el-form label-position="top" size="default">
      <el-form-item label="状态名称">
        <el-input v-model="draft.name" placeholder="例如：选中态" />
      </el-form-item>
    </el-form>

    <div class="block-title">触发条件</div>
    <p class="block-hint">
      同一场景内条件为「且」；多个场景为「或」。字段支持树形选择子字段；选中数组后可填下标。
      <template v-if="repeatListName">
        当前在重复「{{ repeatListName }}」内，可选 <code>index</code> / <code>item.xxx</code>。
      </template>
    </p>
    ...
    <p class="block-hint">仅填写需要覆盖的属性；留空表示沿用基础样式。</p>
    <div class="style-box">
      <StyleEditor
