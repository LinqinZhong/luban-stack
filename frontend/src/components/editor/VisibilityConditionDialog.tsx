...
import './VisibilityConditionDialog.css'

export default function VisibilityConditionDialog({
  open,
  onOpenChange,
  title,
  config,
  ...
  xml?: string
  onSave?: (config: VisibilityConditionConfig) => void
}) {
  ...
  useEffect(() => {
    ...
    const source = config?.scenarios?.length
      ? config
      : createEmptyVisibilityConfig()
    const next = source.scenarios.map((scene) => ({
      ...scene,
      conditions: scene.conditions.map((cond) => ({ ...cond })),
    }))
    const ensured = next.length ? next : [createEmptyScenario(1)]
    setScenarios(ensured)
    setActiveSceneId(ensured[0]?.id ?? '')
  }, [open, config])
