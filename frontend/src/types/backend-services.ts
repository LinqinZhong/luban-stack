...
function normalizeMysqlRef(input: unknown): string {
  ...
  if (isPlainObject(input)) {
    // 旧版 { host, database, ... } 无法可靠映射，置空由用户重选
...
export const HTTP_METHOD_OPTIONS: Array<{ label: string; value: HttpMethod }> = [
  { label: 'GET', value: 'GET' },
...
export const SERVICE_API_PARAM_LOCATION_OPTIONS: Array<{
  label: string
  value: ServiceApiParamLocation
}> = [
  { label: 'query', value: 'query' },
  { label: 'param', value: 'param' },
  { label: 'httpHeader', value: 'httpHeader' },
...
export interface ProcessorMethod {
  ...
  /**
   * 调试入参（按参数名持久化；对象/数组以 JSON 可解析值存储）
   * 仅数据层调试面板使用
   */
  debugParams: Record<string, unknown>
  /** 业务层等方法工作流（一期先业务层使用） */
